...
class RegConvEmbeddingModel(torch.nn.Module):
    def __init__(self, input_size, output_size, modulation_mat_size,
        ...
        if use_label:
            ...
            self._label_representations.weight.requires_grad = False
    ...
    def forward(self, task, params=None, return_task_embedding=False, detached_features=None, is_training=True):
        ...
        # modulation_mat = self._modulation_mat_projection(torch.diag(modulation_mat.squeeze()))
        ...
        modulation_mat = self._modulation_mat_generator(embedding_input)
        modulation_mat = self._modulation_mat_projection.weight.t() + modulation_mat
        ...
        # max_norm = torch.max(
        #         torch.norm(modulation_mat, p=2, dim=1))
        # modulation_mat = modulation_mat.div(max_norm)
...
# the embedding model does not take into account of the labelling task.y?
class DualRegConvEmbeddingModel(torch.nn.Module):
    def __init__(self, input_size, output_size, modulation_mat_size,
                 hidden_size=128, num_layers=1, num_classes=None, use_label=False,
                 convolutional=False, num_conv=6, num_channels=32, num_channels_max=256,
                 rnn_aggregation=False, linear_before_rnn=False, from_detached_features=False,
                 embedding_pooling='max', batch_norm=True, avgpool_after_conv=False,
                 num_sample_embedding=0, sample_embedding_file='embedding.hdf5', original_conv=False, common_subspace_dim=400,
                 img_size=(1, 28, 28), modulation_mat_spec_norm=5., verbose=False, detached_features_size=None):

        super(DualRegConvEmbeddingModel, self).__init__()
        self._input_size = input_size
        self._output_size = output_size
        self._hidden_size = hidden_size
        self._num_layers = num_layers
        ...
        if self._linear_before_rnn:
            ...
            linear_input_size = self.compute_input_size(
                1, 3, 2, self.conv[self._num_layer_per_conv*(self._num_conv-1)].out_channels)
            rnn_input_size = linear_input_size
        elif self._avgpool_after_conv:
            # avg pool across h and w before feeding into rnn
                rnn_input_size = self.conv[self._num_layer_per_conv*(self._num_conv-1)].out_channels
        else:
        ...
        if self._rnn_aggregation:
            if self._linear_before_rnn:
                self.linear = torch.nn.Linear(linear_input_size, rnn_input_size)
                self.relu_after_linear = torch.nn.ReLU(inplace=True)
            self.rnn = torch.nn.GRU(rnn_input_size, rnn_input_size,
                                    num_layers, bidirectional=self._bidirectional)
            embedding_input_size = rnn_input_size*(2 if self._bidirectional else 1)
        else:
            self.rnn = None
            embedding_input_size = rnn_input_size
            self.linear = torch.nn.Linear(rnn_input_size, embedding_input_size)
            self.relu_after_linear = torch.nn.ReLU(inplace=True)
    ...
    def forward_old(self, task, params=None, return_task_embedding=False, features=None, is_training=True):
        ...
        if self._rnn_aggregation:
            ...
            batch_size = x.size(0)
            h0 = torch.zeros(self._num_layers*(2 if self._bidirectional else 1),
                            batch_size, x.size(-1), device=self._device)
            if self._linear_before_rnn: 
                x = F.relu(self.linear(x))
            inputs = x.view(1, x.size(0), -1)
            output, hn = self.rnn(inputs, h0)
        ...
        # print("alpha", modulation_mat)
        modulation_mat = modulation_mat.t() @ features
        # print("modulation_mat", modulation_mat.shape)
        # modulation_mat = F.normalize(modulation_mat, dim=1)
        # print("modulation_mat", torch.norm(modulation_mat, dim=1))
        
        # print("modulation_mat", torch.norm(modulation_mat, dim=1))
        
        # modulation_mat = torch.svd(modulation_mat)[-1].t()
        # reg_strength = torch.sigmoid(self._reg_strength(embedding_input)).squeeze()
        # modulation_mat = modulation_mat / torch.norm(modulation_mat).item()
        # print("reg_strength", reg_strength, np.sqrt(modulation_mat.shape[1] / modulation_mat.shape[0]))
        # print("modulation_mat", torch.norm(modulation_mat, dim=1))
        # max_norm = torch.max(
        #         torch.norm(modulation_mat, p=2, dim=1))
        # modulation_mat = modulation_mat.div(max_norm)
        # reg_strength = torch.norm(modulation_mat).item()
        # modulation_mat = np.sqrt(modulation_mat.shape[1] / modulation_mat.shape[0]) * modulation_mat            
        
        if return_task_embedding:
            return (modulation_mat, 1.), embedding_input
        else:
    ...
    def forward(self, task=None, params=None, return_task_embedding=False, features=None, is_training=True):
        ...
        if return_task_embedding:
            ...
            return (modulation_mat, sigma)


    def to(self, device, **kwargs):
        self._device = device
        super(DualRegConvEmbeddingModel, self).to(device, **kwargs)
