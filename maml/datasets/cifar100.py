...
class Cifar100MAMLSplit():
    def __init__(self, root, train=True,
        ...
        if self._train:
            print('Cifar using train')
            self._images = torch.FloatTensor(self._dataset['data']['train'].reshape([-1, 3, 32, 32])) / 255.0
            self._labels = torch.LongTensor(self._dataset['label']['train'])
        else:
            print('Cifar using test')
            self._images = torch.FloatTensor(self._dataset['data']['test'].reshape([-1, 3, 32, 32])) / 255.0
